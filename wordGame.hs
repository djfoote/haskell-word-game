...
prompt = "Enter a word to get its score, h for a hint, or q to quit."
...
makeDeductions :: [Word] -> [Char] -> ([Char], [Char])
makeDeductions subsets letters = 
    let inAll c = and [c `elem` sub | sub <- subsets]
        notInAll c = and [not $ c `elem` sub | sub <- subsets]
        present = filter inAll letters
        notPresent = filter notInAll letters
    in (present, notPresent)

printDeductions :: [Word] -> IO ()
printDeductions subsets = do
    let (present, notPresent) = makeDeductions subsets letters
    when (not $ null present) $
        putStrLn $ "The following letters must be present: " ++ present
    when (not $ null notPresent) $
        putStrLn $ "The following letters must not be present: " ++ notPresent
    when (null present && null notPresent) $
        putStrLn "No deductions can be made at this time. Try some more words."
...
mainLoop master word subsets = do
    ...
    if guess == "h" 
        then do
            printDeductions subsets
            mainLoop master word subsets
        else do
